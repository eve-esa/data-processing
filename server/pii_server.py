'''
Batching doesnot work in this script.
'''
...
from presidio_analyzer import EntityRecognizer, AnalysisExplanation
...
class FlairRecognizer(EntityRecognizer):
    """Optimized Flair recognizer that reuses loaded model"""
...
class PIILitAPI(ls.LitAPI):
    def setup(self, device):
        ...
        print("set up up PII Removal API...")
        ...
        registry.remove_recognizer("SpacyRecognizer") # remove the spacy recognizer
        ...
        self.email_pattern = re.compile(r'([-a-zA-Z0-9.`?{}]+@\w+(?:\.\w+)+)') # optimal regex
    ...
    def predict(self, request: PIIRequest, context) -> PIIResponse:
        ...
        try:
            ...
            torch.cuda.empty_cache()  # clear gpu cache or high gpu spikes
    ...
    def encode_response(self, response: PIIResponse, context):
        """Encode the response"""
        return response.dict() # is this deprecated? check alt
    ...
    def _anonymize_text(self, text: str, results: List[RecognizerResult]) -> str:
        ...
        # covered spans to avoid overlaps
        covered_spans = set()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description = "PII Removal")
