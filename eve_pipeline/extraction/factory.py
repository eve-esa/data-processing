...
class ExtractorFactory:
    ...
    This factory automatically selects the correct extractor based on file extensions.
    
    Supported file types:
    - PDF: .pdf (uses Nougat or Marker for OCR/parsing)
    - XML: .xml (structured XML parsing)
    - HTML: .html, .htm (web content extraction with trafilatura)
    - Text: .txt, .text, .md, .markdown (plain text/markdown)
    - CSV: .csv, .tsv (tabular data parsing)
    
    Example:
        factory = ExtractorFactory()
        extractor = factory.get_extractor("document.pdf")
        if extractor:
    ...
    _EXTRACTORS: ClassVar[dict[str, type[ExtractorBase]]] = {
        "pdf": PDFExtractor,
        "xml": XMLExtractor,
        "html": HTMLExtractor,
        "htm": HTMLExtractor,
    ...
    def get_extractor(
        ...
        **kwargs,
        ...
        """Get appropriate extractor for file based on its extension.

        Args:
            file_path: Path to file (local or S3). Extension determines the extractor type.
            **kwargs: Additional arguments for extractor initialization (e.g., method="nougat" for PDFs).

        Returns:
            Extractor instance configured for the file type, or None if file type is unsupported.
            
        Examples:
            # PDF extraction with specific method
            extractor = factory.get_extractor("doc.pdf", method="marker")
            
            # HTML extraction
            extractor = factory.get_extractor("page.html")
            
            # Check if file type is supported
            if factory.get_extractor("unknown.xyz") is None:
                print("Unsupported file type")
        """
    ...
    def get_supported_formats(self) -> list[str]:
        ...
        Example:
            ...
            # Output: "Supported: csv, html, htm, md, pdf, txt, xml"
        """
        return sorted(set(self._EXTRACTORS.keys()) | set(self._custom_extractors.keys()))
